...
import { isFunction } from '@ptdgrp/shared';
...
interface Col<T = unknown> {
    ...
    expander?:
        | boolean
        | ((expanded: boolean, record: TreeNode<T>) => ReactNode);
            ...
            <tr role="row">
                {cols.map((col) => (
                    <th
                        ...
                        key={col.key}
                        className={clsx(
                            'z-10 border border-t-0 border-gray-100 bg-gray-100 px-3 py-2.5 text-left text-sm leading-[1] font-bold text-gray-700',
...
const TableBody: FC<{
    ...
    return (
        <tbody role="rowgroup">
            {rows.length === 0 && (
                <tr className="border-r border-l border-gray-100">
                    <td
                        colSpan={cols.length}
...
const TableBodyRow: FC<{
    ...
    return (
        <tr
            className={clsx(
                'border-r border-l border-gray-100 hover:bg-gray-50',
                row.className,
            )}
        >
            {cols.map((col) => (
...
const TableBodyCol: FC<{
    ...
    const expander = col.expander && !row.leaf && (
        <button
            className="mr-1 -ml-2 inline-flex h-4 w-4 items-center justify-center align-middle text-gray-400 hover:text-gray-600"
    ...
    return (
        <td
            role="cell"
            className={clsx(
                'box-border border-b border-gray-200 px-3 py-2 text-sm',
                col.className,
                col.bodyClassName,
            )}
        >
