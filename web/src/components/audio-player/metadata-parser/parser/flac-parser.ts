...
export class FlacParser {
    ...
    public static async is(reader: RemoteReader): Promise<boolean> {
        return reader
            .peek(4)
            .then((bytes) => bytes_equals(bytes, FLAC_SIGNATURE));
    }
}
...
export class Block {
    ...
    public static async new(reader: RemoteReader): Promise<Block> {
        const [is_last, id] = await (async () => {
            const is_last = await reader
                .peek(1)
                .then((bytes) => bytes[0] >> 7 == 1);
            if (is_last) {
                return [
                    true,
                    await reader.read_next_u8().then((num) => num & 0x0f),
                ];
            } else {
