        ...
        async ({ mode, filesOrEntries }) => {
            await new Promise<void>((resolve, reject) => {
                ...
                const timer = window.setTimeout(() => {
                    ...
                    reject(
                        new Error('Failed open upload page, reason: timeout'),
                    );
                }, 1000);
        ...
        <div className={clsx('mb-6 w-full pt-3', className)} {...props}>
            <div className="relative flex items-end gap-1">
                ...
                <textarea
                    ...
                    className="h-full w-auto flex-1 resize-none appearance-none border-none bg-gray-100 p-3 text-sm shadow-none outline-0 outline-none"
                    rows={1}
                />
                <div className="flex h-[2.785rem] items-center">
                    <button
                        className="rounded p-2 text-indigo-600 select-none active:bg-gray-200 active:text-indigo-800 disabled:text-gray-300"
                        disabled={transmitting || !transmittable}
                        onClick={handlers.send}
                        aria-label={i18n._('Send')}
                    >
                        <SendHorizonalIcon className="stroke-background h-6 w-6 fill-current stroke-1" />
                    </button>
                </div>
            </div>
        </div>
