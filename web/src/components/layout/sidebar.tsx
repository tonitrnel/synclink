...
const NavItem = ({
    ...
    <li className="h-[39px] w-[196px] text-sm">
        <button
            onClick={onClick}
            className={clsx(
                'flex h-full w-full cursor-pointer items-center space-x-3 rounded p-3 transition-colors',
                active
                    ? 'bg-gray-700/10 text-black'
                    : 'text-black/70 hover:bg-black/5 hover:text-black',
            )}
        >
            <span className={`${active ? 'text-current' : 'text-black/50'}`}>
                {icon}
            </span>
...
export const Sidebar: FC<SidebarProps> = () => {
    ...
    const stats = useMemo(() => {
        ...
        return {
            ...
            percentage: `(${Math.floor(
                (data.disk_usage /
                    (data.storage_quota - data.default_reserved)) *
                    100,
            )})%`,
    ...
    return (
        <aside className="relative w-[342px] bg-gray-300/90 bg-cover bg-center p-4">
            {/*<div className="absolute inset-0 bg-black/20 backdrop-blur-sm" />*/}
            <div className="relative z-10 flex h-full flex-col p-4">
                <div className="mb-12">
                    <LogoIcon className="h-20 w-20 rounded-md" />
                    <h1 className="-mt-4 text-2xl font-bold">Ephemera</h1>
                ...
                <div className="mt-auto flex flex-col gap-2 text-sm text-black/80">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <span className="block w-24 leading-none text-black/60">
                                {stats?.disk_usage || '-'}
                            </span>
                        </div>
                    </div>
