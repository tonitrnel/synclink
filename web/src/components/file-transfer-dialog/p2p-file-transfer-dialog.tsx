...
export const P2pFileTransferDialog: FC<{
    ...
    return (
        ...
        >
            <Dialog.Header>
                <Dialog.Title>
                    {i18n._('Peer to peer file transfer')}
                </Dialog.Title>
                <Dialog.Description className="sr-only">
                    {i18n._(
                        'Quickly share files with other client who open this page',
                    )}
                </Dialog.Description>
            </Dialog.Header>
            <Dialog.Content>
...
const ClientItem: FC<{
    isDialog: boolean;
    isCurrent: boolean;
    conn: ExtractSchemaType<typeof useSSEConnectionsQuery, 'Response'>[number];
    ...
    return (
        <li
            className={clsx(
                'mt-8 flex justify-between gap-2 first-of-type:mt-0',
            )}
        >
            <div>
                <div className="flex">
...
const ClientSetup: FC<{
    isDialog: boolean;
    currentConnection: ExtractSchemaType<
        typeof useSSEConnectionsQuery,
        'Response'
    >[number];
    otherConnections: ExtractSchemaType<
        typeof useSSEConnectionsQuery,
        'Response'
    >;
                    ...
                    <Select
                        ...
                        onValueChange={(value) =>
                            withProduce(
                                ...
                                (draft) =>
                                    void (draft.protocol =
                                        value as ProtocolPriority),
                            )
                        }
                    >
                        <Select.Trigger className="w-[8rem]">
                            <Select.Value
                                placeholder={i18n._('Select a protocol')}
                            />
                        </Select.Trigger>
                        <Select.Content>
            ...
            <section>
                <ul className="my-2 max-h-[18rem] overflow-y-auto text-sm">
                    {otherConnections.length > 0 ? (
                        ...
                        <li className="py-3 text-gray-300 select-none">
                            {i18n._('No data')}
                        </li>
                    )}
                </ul>
...
const ConnectionControl: FC<{
    ...
    const statusTexts = useMemo(
        () => ({
            [ConnectionStatus.WaitingForAcceptance]: i18n._(
                'Waiting for acceptance',
            ),
            [ConnectionStatus.Connecting]: t(
                i18n.i18n,
            )`Connecting, using ${state.protocol} protocol`,
            [ConnectionStatus.TestingAvailability]: i18n._(
        ...
        return notifyManager.batch(
            ...
            notifyManager.on('P2P_SIGNALING', async (value) => {
                ...
                if (value[0] == 0) {
                    if (value[1].type === 'offer') {
                        if (!metadata.requestId || !metadata.clientId)
                            return void 0;
                        const webrtc = new P2PRtc(
                            metadata.requestId,
                            metadata.clientId,
            ...
            () => {
                ...
                if (metadata.requestId && mode == 'sender' && !conn)
                    discardRequest(metadata.requestId, metadata.clientId!);
            },
        );
    }, [
        createRequest,
                ...
                <div className="my-10 flex w-full flex-col items-center gap-2">
                    ...
                    <div className="mt-2 text-left">
                        <p className="font-bold">
                            {i18n._('Oh! An error has occurred')}
                        </p>
            ...
            try {
                ...
                while (true) {
                    ...
                    const buf = new ArrayBuffer(
                        value.length + ACK_PACKET_LENGTH,
                    );
                    createACKPacket(fileSeq, packetSeq, buf);
                    new Uint8Array(buf, ACK_PACKET_LENGTH, value.length).set(
                        value,
                    );
                    ...
                    if (currentTime - previousUpdateTime > 1000) {
                        ...
                        withProduce(setDeliveryItems, (draft) => {
                            ...
                            if (!target) return void 0;
                            target.progress = Math.ceil(
                                (transmitted / total) * 100,
                            );
                ...
                async pull(controller) {
                    ...
                    try {
                        ...
                        while (
                            ...
                            if (!value) continue;
                            const [_fileSeq, _packetSeq2] =
                                parseACKPacket(value);
            ...
            <div className="mt-4 w-full text-sm">
                ...
                <ul className="mt-2 max-h-[10rem] w-full overflow-y-auto">
                    {deliveryItems.length == 0 ? (
                        <li className="mt-2 ml-1 text-gray-300">
                            {i18n._('No data')}
                        </li>
                    ) : (
