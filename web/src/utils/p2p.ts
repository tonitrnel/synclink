...
export abstract class RTCImpl extends EventBus<RTCEvents> {
    ...
    public ping = async (): Promise<number> => {
        ...
        const replyTime = await new Promise<number>((resolve, reject) => {
            ...
            const timerId = window.setTimeout(() => {
                ...
                reject(
                    new Error(
                        'Ping test timed out after ' + timeoutDuration + ' ms',
                    ),
                );
            }, timeoutDuration);
            const pongHandler = (buffer: ArrayBuffer) => {
                ...
                if (
                    new DataView(receivedBytes.buffer, 4, 2).getUint16(
                        0,
                        true,
                    ) !== seq
                ) {
        ...
        this.rtt = Math.max(
            Math.ceil((replyTime - startTime + this.rtt) / 2),
            0,
        );
    ...
    protected pong = (buffer: ArrayBuffer) => {
        ...
        if (
            bytes
                .subarray(0, 4)
                .some((byte, index) => byte != PING_OPCODE[index])
        ) {
            return void 0;
        }
...
export class P2PSocket extends RTCImpl {
    ...
    public async *recv(): AsyncGenerator<ArrayBuffer, void> {
        ...
        if (!this.established) {
            await new Promise<void>((resolve) =>
                this.once(PacketFlag.PROXY_CONNECTION_ESTABLISHED, () =>
                    resolve(),
                ),
            );
        }
