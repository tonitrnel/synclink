...
export class RemoteReader extends Reader {
  ...
  constructor(public readonly url: string, private readonly chunk_size = 4096) {
    super();
  }
  ...
  public async init(): Promise<void> {
    ...
    this._total = (() => {
      ...
      if (!ranges || ranges === '') throw new Error('?');
  ...
  private async ensure(end: number) {
    if (end >= this._total) {
      throw new Error('Requested data exceeds the total available data size.');
    }
  ...
  public read_next_u16(is_big_endian: boolean): Promise<number> {
    return this.read(2).then((bytes) => bytes_to_integer(bytes, is_big_endian));
  }

  public read_next_u32(is_big_endian: boolean): Promise<number> {
    return this.read(4).then((bytes) => bytes_to_integer(bytes, is_big_endian));
  }
  ...
  public read_next_u64(is_big_endian: boolean): Promise<number> {
    return this.read(8).then((bytes) => bytes_to_integer(bytes, is_big_endian));
  }
