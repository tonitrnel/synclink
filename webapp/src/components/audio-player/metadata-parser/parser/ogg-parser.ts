...
export class OggParser {
  ...
  public static async is(reader: RemoteReader): Promise<boolean> {
    return reader.peek(4).then((bytes) => bytes_equals(bytes, OGG_SIGNATURE));
  }

  public static load_fulldata(segments: Segment[], start: number): Uint8Array {
...
export class Segment {
  ...
  public static async new(reader: RemoteReader): Promise<Segment> {
    if (
      await reader.peek(4).then((bytes) => !bytes_equals(bytes, OGG_SIGNATURE))
    ) {
...
export class VorbisIdentification {
  ...
  public static is_vorbis_format(bytes: Uint8Array): boolean {
    return (
      bytes[0] == 0x01 &&
      bytes_equals(bytes.slice(1, 7), [0x76, 0x6f, 0x72, 0x62, 0x69, 0x73])
    );
  }
}
